...
# commonly rewriting in all scripts.
#
...
#
# Exemple:
# Say, if your script can accept a '--all' option, which can be
# shortened as '-a'.
#
# argsparse_use_option "=all" "
# to be continued
#
...
__args_parse_join_array() {
       # Like the 'join' string method in python, join multiple strings
       # by a char.
       # 1st Parameter: a char
...
argsparse_set_option_without_value() {
	...
	[ $# -ne 1 ] && return 1
	local option=$1
	program_options["$option"]=$((${program_options["$option"]:-0}+1))
}
...
__argsparse_parse_options_no_usage() {
	...
	do
		...
		then
			...
			if [[ "$(declare -p "$possible_values" 2>/dev/null)" = "declare -"[aZ]" $possible_values='("* ]]
			then
...
argsparse_use_option() {
	...
	do
		case "$1" in
			mandatory)
				argsparse_set_option_mandatory "$long_name"
				;;
			*)
				# The default value
